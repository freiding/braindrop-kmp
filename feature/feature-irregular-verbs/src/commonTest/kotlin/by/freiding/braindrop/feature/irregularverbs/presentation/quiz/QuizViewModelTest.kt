...
class QuizViewModelTest {
    ...
    private fun buildViewModel(repository: FakeIrregularVerbRepository): QuizViewModel =
        QuizViewModel(
    ...
    @Test
    fun `loading a quiz populates a question for every unlearned verb`() =
    ...
    fun `selecting the correct answer increments score and marks the question answered`() =
        runTest(testDispatcher) {
            ...
            testDispatcher.scheduler.runCurrent()
            try {
                val answeredVerbId = viewModel.state.value.currentQuestion!!
                    .verb.id
    ...
    @Test
    fun `retrying mistakes restricts the next session to exactly the missed verbs`() =
        runTest(testDispatcher) {
            val verbs = listOf(verbFixture("verb1"), verbFixture("verb2"))
            val repository = FakeIrregularVerbRepository(
                verbsWithProgress = Result.Success(verbs.map { verbWithProgressFixture(it) }),
