...
class GenerateQuizUseCaseTest {
    ...
    @Test
    fun `restrictToVerbIds builds the session strictly from those verbs`() =
        runTest {
    ...
    @Test
    fun `empty unlearned verbs is a valid empty result, not an error`() =
        runTest {
    ...
    @Test
    fun `error loading all verbs propagates before the unlearned branch runs`() =
        runTest {
    ...
    @Test
    fun `error loading unlearned verbs propagates`() =
        runTest {
            val exception = AppException.DatabaseException("boom")
            val repository = FakeIrregularVerbRepository(
                verbsWithProgress = allVerbsWithProgress,
    ...
    @Test
    fun `session size caps the number of questions drawn from unlearned verbs`() =
        runTest {
            val repository = FakeIrregularVerbRepository(
                verbsWithProgress = allVerbsWithProgress,
    ...
    @Test
    fun `each question offers the correct answer among its options`() =
        runTest {
            val repository = FakeIrregularVerbRepository(
                verbsWithProgress = allVerbsWithProgress,
                unlearnedVerbs = Result.Success(allVerbs),
