...
class IrregularVerbRepositoryImpl(
    ...
    override suspend fun getVerbsWithProgress(): Result<List<VerbWithProgress>> = runCatching {
    ...
    override suspend fun getVerbDetail(verbId: String): Result<VerbWithProgress> = runCatching {
    ...
    override suspend fun toggleLearned(verbId: String): Result<Unit> = runCatching {
        val row = progressDataSource.getByVerbId(verbId)
    ...
    override suspend fun recordAnswer(verbId: String, isCorrect: Boolean): Result<Unit> = runCatching {
        val row = progressDataSource.getByVerbId(verbId)
        progressDataSource.upsertProgress(
            verbId = verbId,
            isLearned = row?.is_learned == 1L,
    ...
    override suspend fun getUnlearnedVerbs(): Result<List<IrregularVerb>> = runCatching {
